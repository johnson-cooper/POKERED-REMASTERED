...
static u8 legacy_v3_checksum(const LegacyV3SaveData *data) {
    const u8 *bytes = (const u8 *)data;
    u8 sum = 0;
    for (u32 i = 4; i < offsetof(LegacyV3SaveData, checksum); i++)
        sum = (u8)(sum + bytes[i]);
    return sum;
}
...
bool8 save_exists(void) {
    ...
    if (GBA_SRAM[4] == 3) {
        LegacyV3SaveData data;
        for (u32 i = 0; i < sizeof(LegacyV3SaveData); i++)
            ((u8 *)&data)[i] = GBA_SRAM[i];
...
bool8 save_read(SaveData *out) {
    ...
    if (GBA_SRAM[4] == 1) {
        ...
        out->checksum = legacy.checksum;
    } else if (GBA_SRAM[4] == 3) {
        LegacyV3SaveData legacy;
        for (u32 i = 0; i < sizeof(LegacyV3SaveData); i++)
            ((u8 *)&legacy)[i] = GBA_SRAM[i];
        out->magic[0] = legacy.magic[0];
        out->magic[1] = legacy.magic[1];
